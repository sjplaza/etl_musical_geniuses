...
function updateToolTip(chosenX, circlesGroup) {
    ...
    let toolTip = d3.tip()
        ...
        .offset([80, -60])
        .html(function(d) {
            return (`${d.artists}<br>${label} ${d[chosenX]}`);
        });
...
d3.csv('data.csv').then(function(MusicData) {
    ...
    MusicData.forEach(function(data) {
        ...
        data.musicstuff = +data.musicstuff
    })

    let xLinearScale = xScale(MusicData, chosenX);

    let yLinearScale = d3.scaleLinear()
        .domain([0, d3.max(MusicData, d=>d.musicstuff)])
        .range([height, 5]);
    ...
    let circlesGroup = chartGroup.selectAll("circle")
        ...
        .attr("cy", d => yLinearScale(d.musicstuff))
        .attr("r", 6)
        .attr("fill", "purple")
        .attr("opacity", "0.5")
